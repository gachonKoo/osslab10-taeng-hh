...
import os
import glob
import numpy as np # 자가 테스트를 위해 numpy 추가
...
def load_image(file_path):
    ...
    OpenCV는 기본적으로 BGR 형식으로 이미지를 로드합니다.
    ...
    # cv2.IMREAD_COLOR (1)은 컬러 이미지를 로드하는 플래그입니다.
...
def save_image(image, file_path, quality=95):
    ...
    Args:
        ...
        quality (int): JPEG 압축 품질 (0-100). PNG의 경우 압축 레벨로 사용됩니다.
    ...
    if extension in ('.jpg', '.jpeg'):
        # JPEG 품질 설정 (cv2.IMWRITE_JPEG_QUALITY, 0-100)
        params = [cv2.IMWRITE_JPEG_QUALITY, quality]
    elif extension == '.png':
        # PNG 압축 레벨 설정 (cv2.IMWRITE_PNG_COMPRESSION, 0-9)
...
def get_image_files(folder_path):
    ...
    Args:
        folder_path (str): 이미지 파일들을 탐색할 폴더의 경로.
        
    Returns:
        list[str]: 이미지 파일 경로 문자열 리스트.
    """
